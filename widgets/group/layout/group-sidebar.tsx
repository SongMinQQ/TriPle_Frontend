...
export function GroupSidebar({ groupId }: GroupSidebarProps) {
  ...
  const {
    ...
    isDeletingGroup,
    requestDeleteGroup,
  } = useGroupManageOptionAction(groupId);

  const membershipAction = groupMenu?.role
    ? getGroupMembershipActionFromRole(groupMenu.role)
    : null;
  const membershipActionControl = membershipAction
    ? getGroupMembershipActionControl({
            ...
            <div className="mt-3 flex flex-col items-center lg:ml-4 lg:mt-0 lg:flex-1 lg:items-start">
              ...
              {membershipAction ? (
                <GroupMembershipActionButton
                  action={membershipAction}
                  className="mt-2"
                  disabled={membershipActionControl?.disabled ?? false}
